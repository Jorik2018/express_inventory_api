  ...
  try {
    ...
    switch (type) {
      ...
      case "inventory":
        ...
        movementDetail.forEach(
          (
            md: MovementDetail & { movement: Movement; inventory?: Inventory }
          ) => {
    ...
    try {
      response = await axios.post(
        "http://web.regionancash.gob.pe/api/jreport/",
        form,
        {
          headers: {
            ...form.getHeaders(),
      ...
      res
        .status(400)
        .send("Error in the generation of report in the Java Endpoint");
    }
  } catch (error) {
    console.log(error);
...
router.post("/movement", validateToken, async (req: Request, res: Response) => {
  try {
    ...
    data.unit_organic_destiny =
      data.unit_organic_destiny === undefined
        ? data.unit_organic
        : data.unit_organic_destiny;
    data.local_destiny =
      data.local_destiny === undefined ? data.local : data.local_destiny;
    console.log(data);
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      if (existingInventory) {
        ...
        return res
          .status(400)
          .json({ error: "El patrimonial_code ya está registrado." });
      }
      ...
      // Obtenemos el valor de id de la solicitud HTTP
...
router.put(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      let data: Inventory = req.body.data;
      data.user_id = req.body.user_id;
      data.patrimonial_code =
        data.patrimonial_code === "" ? "S/C" : data.patrimonial_code;
